...
把指针运算符 * 应用到 var 上是完全可以的，但修改 var 的值是非法的。这是**因为 var 是一个指向数组开头的常量，不能作为左值**。
...
当返回一个引用时，要注意被引用的对象不能超出作用域。所以**返回一个对局部变量的引用是不合法的，但是，可以返回一个对静态变量的引用**。
...
int main( )
{
   ...
   // 对象 obj 作为参数传递给了 display 函数，这会触发拷贝构造函数的调用，也就是会新建一个 Line 对象并将原对象 obj 的值复制到新建的对象中。当 display 函数执行完毕，会自动调用新建的对象的析构函数来释放内存
   display(line);
 
   // 对象 line 在函数返回时会被销毁，这会调用 line 的析构函数
   return 0;
}
```
...
```c++
double* pvalue  = NULL;
if( !(pvalue  = new double )) // 0被视为假（false）, !0的结果为true
{
...
```
Value of __LINE__ : 6
Value of __FILE__ : H:\Study\c-practice\C++Project\runoob.cpp
Value of __DATE__ : Jun 20 2023
Value of __TIME__ : 09:09:07
```
